...
public class PlayerListItem extends MultiVersionPacketV17 {
    ...
    public void v17Read(final ByteBuf buf, final ProtocolConstants.Direction direction, final int protocolVersion) {
        ...
        items[n] = item2;
        item2.displayName = (item2.username = DefinedPacket.readString(buf));
        this.action = (buf.readBoolean() ? Action.ADD_PLAYER : Action.REMOVE_PLAYER);
        item2.ping = buf.readShort();
    }
