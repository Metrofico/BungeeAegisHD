...
public final class UserConnection implements ProxiedPlayer {
    ...
    @Getter
    @Setter
    private List<PluginMessage> delayedPluginMessages = new BoundedArrayList<>(Settings.IMP.AEGIS_SETTINGS.MAX_DELAYED_PLUGIN_MESSAGES_LIST_SIZE); //BotFilter
    /*========================================================================*/
    @Getter
    @Setter
    ...
    public void init() {
        ...
        forgeClientHandler.setFmlTokenInHandshake(this.getPendingConnection().getExtraDataInHandshake()
    ...
    public ServerInfo updateAndGetNextServer(ServerInfo currentTarget) {
        ...
        if (next == null) {
            if (getPendingConnection() != null && getPendingConnection().getListener() != null) {
                next = this.bungee.getServerInfo(getPendingConnection()
                        .getListener().getFallbackServer());
            }
        }
    ...
    public void connect(ServerInfo info, final Callback<Boolean> callback, final boolean retry,
        ...
        ServerConnectRequest.Builder builder = ServerConnectRequest.builder().retry(retry)
                .reason(reason).target(info);
        if (callback != null) {
            ...
            builder.callback(new Callback<ServerConnectRequest.Result>() {
                ...
                public void done(ServerConnectRequest.Result result, Throwable error) {
                    callback
                            .done((result == ServerConnectRequest.Result.SUCCESS) ? Boolean.TRUE : Boolean.FALSE,
                                    error);
                }
            });
        }

        builder.connectTimeout(bungee.getConfig().getTimeout());

        connect(builder.build());
    }

    public void connectNoQueue(ServerInfo info, final Callback<Boolean> callback, final boolean retry,
                               ServerConnectEvent.Reason reason) {
        Preconditions.checkNotNull(info, "info");
    ...
    public void connectNoQueue(final ServerConnectRequest request) {
        Preconditions.checkNotNull(request, "request");

        final Callback<ServerConnectRequest.Result> callback = request.getCallback();
        ServerConnectEvent event = new ServerConnectEvent(this, request.getTarget(),
                request.getReason());
        ...
        final BungeeServerInfo target = (BungeeServerInfo) event
                .getTarget(); // Update in case the event changed target
        ...
        ChannelFutureListener listener = new ChannelFutureListener() {
            ...
            public void operationComplete(ChannelFuture future) throws Exception {
                ...
                if (!future.isSuccess()) {
                    ...
                    if (request.isRetry() && def != null && (getServer() == null || def != getServer()
                            .getInfo())) {
    ...
    /*if (Settings.IMP.AEGIS_SETTINGS.LICENSE.equals("yourLicenseKeyHere")) {
      System.exit(201);
      return;
    }*/
        ...
        ChannelInitializer initializer = new ChannelInitializer() {
            ...
            protected void initChannel(Channel ch) throws Exception {
                PipelineUtils.BASE.initChannel(ch);
                ch.pipeline().addAfter(PipelineUtils.FRAME_DECODER, PipelineUtils.PACKET_DECODER, new MinecraftDecoder(Protocol.HANDSHAKE, false, getPendingConnection().getVersion(), false));
    ...
    public void sendMessage(ChatMessageType position, BaseComponent... message) {
        ...
        if (position == ChatMessageType.ACTION_BAR && this.getPendingConnection().getVersion() >= 47) {
    ...
    public Locale getLocale() {
        return (locale == null && settings != null) ? locale = Locale
                .forLanguageTag(settings.getLocale().replace('_', '-')) : locale;
    }
    ...
    public MainHand getMainHand() {
        return (settings == null || settings.getMainHand() == 1) ? MainHand.RIGHT
                : MainHand.LEFT;
    }
