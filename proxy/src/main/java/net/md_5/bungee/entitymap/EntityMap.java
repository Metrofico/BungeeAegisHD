    ...
    while ( ( index = packet.readUnsignedByte() ) != 0xFF )
    {
      ...
      switch ( type )
      {
        ...
        default:
          ...
          BungeeCord.getInstance().getLogger().log(Level.WARNING, "There was an error while rewriting entity metadata! PLEASE TRY ENABLING \"disable-entity-metadata-rewrite\" in Aegis/config.yml and restart server.");
          throw new IllegalArgumentException("Unknown meta type " + type + ": Using mods? PLEASE TRY ENABLING \"disable-entity-metadata-rewrite\" in Aegis/config.yml");
      }
    }
