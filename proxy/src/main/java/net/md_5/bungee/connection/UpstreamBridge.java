...
public class UpstreamBridge extends PacketHandler {
    ...
    public UpstreamBridge(ProxyServer bungee, UserConnection con) {
        ...
        con.unsafe().sendPacket(
                BungeeCord.getInstance().registerChannels(con.getPendingConnection().getVersion()));
    ...
    public void disconnected(ChannelWrapper channel) {
        final AuthSystem authSystem = BungeeCord.getInstance().getAegis().getAuthSystem();
        if (authSystem != null) {
            final AuthUser user = authSystem.getUserManager().getUser(con.getUniqueId());
            if (user != null) {
                authSystem.getConcurrentManager().runAsyncSave(() -> {
                    try {
                        authSystem.getDatabase().saveObject(user);
                    } catch (SQLException ex) {
                        this.bungee.getLogger().log(Level.WARNING,
                                "[Aegis Auth] Could not save data of player " + con.getName() + "! " + ex
                                        .getMessage());
                    }
                    return user;
                });
    ...
    public void handle(Chat chat) throws Exception {
        int maxLength =
                (con.getPendingConnection().getVersion() >= ProtocolConstants.MINECRAFT_1_11) ? 256 : 100;
    ...
    public void handle(TabCompleteRequest tabComplete) throws Exception {
        ...
        if (con.getPendingConnection().getVersion() <= ProtocolConstants.MINECRAFT_1_12_2
                && lastTabCompletion > 0 && (now - lastTabCompletion) <= 500) {
            throw CancelSendSignal.INSTANCE;
        }
        ...
        TabCompleteEvent tabCompleteEvent = new TabCompleteEvent(con, con.getServer(),
                tabComplete.getCursor(), suggestions);
        ...
        if (!results.isEmpty()) {
            ...
            if (con.getPendingConnection().getVersion() < ProtocolConstants.MINECRAFT_1_13) {
                ...
                con.unsafe().sendPacket(new TabCompleteResponse(tabComplete.getTransactionId(),
                        new Suggestions(range, brigadier)));
            }
            throw CancelSendSignal.INSTANCE;
        }
    }
