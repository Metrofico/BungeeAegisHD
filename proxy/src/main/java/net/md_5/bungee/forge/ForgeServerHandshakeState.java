...
/**
 * Handshake sequence manager for the Bungee - Forge Server (Downstream/Server Connector) link.
 * Modelled after the Forge implementation.
 */
public enum ForgeServerHandshakeState implements
    IForgeServerPacketHandler<ForgeServerHandshakeState> {
  ...
  WAITINGCACK {
    ...
    public ForgeServerHandshakeState send(PluginMessage message, UserConnection con) {
      ...
      if (message.getData()[0] == -1 && message.getTag()
          .equals(ForgeConstants.FML_HANDSHAKE_TAG)) // transition to COMPLETE after sending ACK
      {
        return this;
      }
