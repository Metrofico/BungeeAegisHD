...
public class ForgeServerHandler {
  ...
  public void handle(PluginMessage message) throws IllegalArgumentException {
    if (!message.getTag().equalsIgnoreCase(ForgeConstants.FML_HANDSHAKE_TAG) && !message.getTag()
        .equalsIgnoreCase(ForgeConstants.FORGE_REGISTER)) {
    ...
    if (this.state == ForgeServerHandshakeState.DONE || state != prevState) // send packets
    {
  ...
  /**
   * Flags the server as a Forge server. Cannot be used to set a server back to vanilla (there would
   * be no need)
   */
