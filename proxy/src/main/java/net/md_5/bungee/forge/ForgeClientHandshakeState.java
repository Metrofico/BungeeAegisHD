...
/**
 * Handshake sequence manager for the Bungee - Forge Client (Upstream) link. Modelled after the
 * Forge implementation. See https://github.com/MinecraftForge/FML/blob/master/src/main/java/cpw/mods/fml/common/network/handshake/FMLHandshakeClientState.java
 */
enum ForgeClientHandshakeState implements IForgeClientPacketHandler<ForgeClientHandshakeState> {

  /**
   * Initiated at the start of a client handshake. This is a special case where we don't want to use
   * a {@link PluginMessage}, we're just sending stuff out here.
   * <p>
  ...
  /**
   * Waiting to receive a client HELLO and the mod list. Upon receiving the mod list, return the mod
   * list of the server.
   * <p>
  ...
  COMPLETE {
    @Override
    public ForgeClientHandshakeState handle(PluginMessage message, UserConnection con) {
      if ((message.getTag().equals("FML|HS") && message.getData()[0] == -1) || message.getTag().equals("FORGE")) {
        ForgeLogger.logClient(ForgeLogger.LogDirection.RECEIVED, this.name(), message);
        con.unsafe().sendPacket(message);
      }
